...
int Verify_Username(char user[], size_t len) {
    ...
    for (int i = 0; i < size; i++) {
        if(!IsLetter(user[i]) && !IsDigit(user[i]) && user[i] != '_') {
...
int Verify_Email(char email[], size_t len) {
    ...
    /* BEGIN MODIFYING CODE HERE */
    int size = Length(email, len);
    
    int name_length = 0;
    int email_at_index = -1;
    for (int i = 0; i < size; i++) {
        if (email[i] == '@') {
            email_at_index = i;
            break;
    ...
    if (IsTopDomain(email, size-4) && email_at_index == -1) {
        name_length = 0;
    ...
    if (IsTopDomain(email, size-4)) {
        domain_size = (size-4) - (email_at_index+1);
    } else {
        domain_size = size - (email_at_index+1);
    }
    ...
    if (IsTopDomain(email, size-4)) {
        if (size - domain_start - 4 > MAX_DOMAIN_LEN) {
    ...
    for (int i = email_at_index; i < size; i++) {
        if (email[i] == '@') {
            ...
            if (email[i+1] != '\0' && !IsLetter(email[i+1])) {
                printf(ERROR_11_EMAIL_DOMAIN_START_INVALID);
                return 0;
            }
        }
    ...
    for (int i = domain_start; i < end_index; i++) {
        if(!IsLetter(email[i]) && !IsDigit(email[i]) && email[i] != '.') {
